...
- (UIActivityIndicatorView *)sepaShowActivityIndicatorInView:(UIView *)inView {
    ...
    tempActivityIndicator.color = appController.appMainColor;
...
- (void)sepaHideActivityIndicator:(UIActivityIndicatorView *)tempActivityIndicator {
...
- (NSArray*)getAllContactFromPhone{
    ...
    for (int i = 0; i < nPeople; i++) {
        ...
        @autoreleasepool {
            ...
            if (!contacts.image) {
                contacts.image = [UIImage imageNamed:@"user_placeholder"];
            }
...
- (NSString *) getDifferenceBetweenTwoDate:(NSString*)startDate toDate:(NSString *)endDate{
    ...
    return [NSString stringWithFormat:@"%lu", hoursBetweenDates];
}
